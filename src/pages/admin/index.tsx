...
export default function AdminPage() {
  ...
  const [{ status: removeStatus }, doRemoveInvitation] = useRequest(
    apiService.removeInvitation
  );
  useEffect(() => {
  ...
  const columns: ColumnsType<InvitationDto> = [
    ...
    },
    {
      ...
      render: (relationship) => {
        return (
          <Typography.Text>
            {relationship === Relationship.BRIDE_GUESTS
              ? "Khách mời cô Tư"
              : "Khách mời của lớp 12A1"}
          </Typography.Text>
        );
      },
    },
  ...
  return (
    <AdminLayout>
      <div className={"flex justify-between items-center"}>
        <div>
          <Typography.Title className={"mb-2"}>Invitations</Typography.Title>
