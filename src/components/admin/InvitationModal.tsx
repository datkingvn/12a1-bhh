...
export const InvitationModal = (props: InvitationModalProps) => {
  ...
  const [{ status: createStatus }, doCreateInvitation] = useRequest(
    apiService.createInvitation
  );
  const [{ status: updateStatus }, doUpdateInvitation] = useRequest(
    ...
    <div>
      ...
      >
        <Form form={form} layout={"vertical"} onFinish={handleSubmitForm}>
          <div className={"flex gap-6"}>
            ...
            >
              <Select
                ...
                options={[
                  {
                    value: Relationship.BRIDE_GUESTS,
                    label: "Khách mời cô Tư",
                  },
                  {
                    value: Relationship.GROOM_GUESTS,
                    label: "Khách mời lớp 12A1",
                  },
                ]}
              />
            </Form.Item>
          ...
          <Form.Item
            label="Thông tin về mối quan hệ"
            name={"description"}
            rules={[{ required: true }]}
          >
            <Input.TextArea
              placeholder="Bổ sung thông tin về mối quan hệ..."
              rows={3}
            />
          </Form.Item>
        </Form>
